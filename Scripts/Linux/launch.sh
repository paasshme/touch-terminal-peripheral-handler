...
if [[ $EUID > 0 ]]; then    
    chromium --kiosk https://localhost:5001/swagger/index.html  --noerrdialogs 
	--disable-infobars --no-default-browser-check --no-experiments --no-pings 
	--silent-debugger-extension-api &
else
    chromium --kiosk https://localhost:5001/swagger/index.html --no-sandbox 
	--noerrdialogs --disable-infobars --no-default-browser-check --no-experiments 
	--no-pings --silent-debugger-extension-api &
fi
